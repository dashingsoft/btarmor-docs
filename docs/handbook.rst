...
如果单独保护某一个函数的堆栈，也可以通过在函数开始的时候调用 ``BTPROT_STACK`` 来
实现
...
如果需要对内存堆的进行保护，那么需要对源代码进行相应的修改，使用 ``btapp.h`` 提
供的宏定义 ``BTMALLOC`` 等来进行内存管理。例如，
...
首先对数据文件进行加密，可以使用命令 :ref:`btmake` 对数据文件进行加密。下面的命
令会对数据文件 ``data.txt`` 进行加密，加密后的文件存放在 ``dist/`` 下面

    btarmor make data.txt

然后在使用 ``mmap`` 分配的内存的时候指定标志 ``MAP_BTARMOR`` 。 例如，
