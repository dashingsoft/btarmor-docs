...
因为字符串默认是处于保护模式，无法被内核访问，而系统调用 `open` 需要把文件名称传
递到内核，下面这个示例中默认模式下是无法正常运行的
...
如果只是个别字符串需要被内核访问，可以在函数中使用 `BTS` 声明字符串常量。被声明
为共享的字符串，会被存放到一个特殊的数据段中，这个数据段允许内核访问，从而避免数
据共享问题。这种方式是把字符串拷贝到一个共享缓冲区中，因为该缓冲区是全局的，所以
但是这种方式无法应用于多线程中。例如
...
如果需要单独保护某一个函数的堆栈，也可以通过在函数开始的时候调用 `BTPROT_STACK`
来实现
...
等直接分配内存，而是需要使用 `btapp.h` 提供的宏定义来 `BTMALLOC` 等来实现。
